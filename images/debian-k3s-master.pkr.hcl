...
source "hyperv-iso" "debian" {
  ...
  boot_command       = ["<wait>c<wait>", 
                      ...
                      "url=http://{{ .HTTPIP }}:{{ .HTTPPort }}//img-base.cfg ", 
...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "echo '${var.packer.password}' | sudo -S apt update && echo '${var.packer.password}' | sudo -S apt install curl python3 -y",
      "echo '${var.packer.password}' | sudo -S sh -c \"curl -sfL https://get.k3s.io | sh -\"",
